...
/**
 * Record Troll Combat Transcript with Deterministic RNG
 * 
 * Plays the TypeScript game with seed 12345 and records the actual
 * combat sequence for troll combat.
 */
...
const transcript = createTranscriptTemplate({
  id: '22-troll-combat-deterministic',
  name: 'Troll Combat (Deterministic)',
  description: 'Detailed troll combat sequence with deterministic RNG (seed 12345)',
  category: 'combat',
  priority: 'high',
});
...
const commands = [
  ...
  { cmd: 'e', notes: 'Try to go east (should be blocked)' },
  ...
  { cmd: 'e', notes: 'Go east from troll room (after troll dies)' },
];
...
// Save transcript
const outputPath = '.kiro/transcripts/high/22-troll-combat-deterministic.json';
