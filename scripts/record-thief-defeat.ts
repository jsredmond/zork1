...
/**
 * Record Thief Defeat Transcript with Deterministic RNG
 * 
 * Plays the TypeScript game with seed 12345 and records the actual
 * combat sequence for the thief defeat (actually troll combat).
 */
...
const transcript = createTranscriptTemplate({
  id: '21-thief-defeat-deterministic',
  name: 'Troll Combat Sequence 2 (Deterministic)',
  description: 'Troll combat with deterministic RNG (seed 12345) - originally mislabeled as thief defeat',
  category: 'combat',
  priority: 'high',
});
...
const commands = [
  ...
  { cmd: 'kill troll with sword', notes: 'Attack troll - round 9' },
  { cmd: 'kill troll with sword', notes: 'Attack troll - round 10' },
  { cmd: 'e', notes: 'Go east from troll room' },
];

console.log('=== Recording Troll Combat (Thief Defeat) with Deterministic RNG (seed 12345) ===\n');
...
// Save transcript
const outputPath = '.kiro/transcripts/high/21-thief-defeat-deterministic.json';
